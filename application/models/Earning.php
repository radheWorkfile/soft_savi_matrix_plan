...
class Earning extends CI_Model
{
    ...
    public function reg_earning($userid, $sponsor, $packageid, $need_topup = true, $qty = 1)
    {
        ...
        if ($get_topup > "0" or $need_topup === false) :
            ...
            if ($data->direct_income > "0.00" && trim($sponsor) !== '') {
                $direct_income = ($data->direct_income * $data->prod_price) / 100;
            ...
            if (trim($data->level_income) !== "") {
                ...
                foreach ($ex as $e) {
                    $e = trim($e);
                    $level_income_amount = ($e * $data->prod_price) / 100;
                    if ($i == 0) {
                        $pay_position = $position;
                    } else {
                        $pay_position = $this->find_level_sponsor($position, $i);
                    }
                    if ($pay_position > 0 && $e > 0) {
                        $this->pay_earning($pay_position, $userid, 'Level Income', $level_income_amount);
                    }
                    $i++;
                }
            }
    ...
    function donation_dist($userid, $sponsor, $amount, $need_topup = true, $qty = 1)
    {
        $direct_income_percent = 18;
        if ($amount > 0  && $sponsor > 0) {

            $direct_income = ($direct_income_percent * $amount) / 100;
            $this->pay_earning($sponsor, $userid, 'Donation Referral Income', $direct_income);
        }

        ## NOW Level Income
        $position = $this->db_model->select('position', 'member', array('id' => $userid));
        $ex = array(9, 9, 9, 9, 9, 9, 4.5, 4.5, 4.5, 4.5);
        $i  = 0;
        foreach ($ex as $e) {
            $e = trim($e);
            $level_income_amount = ($e * $amount) / 100;
            if ($i == 0) {
                $pay_position = $position;
            } else {
                $pay_position = $this->find_level_sponsor($position, $i);
            ...
            if ($pay_position > 0 && $e > 0) {
                $this->pay_earning($pay_position, $userid, 'Donation Level Income', $level_income_amount);
            }
            $i++;
        }
 ...
 */
    ...
    function check_donation_eligibility()
    {
        ...
        if ($total_earning >= 4000 && $last_donation_amount == '') {
            $donation_link = "<p class='text-center'><a href=" . site_url('member/donate_now/') . " class='btn btn-success '> Donate Now</a><p>";
            $redonate = 'yes';
            $donation_amount = 500;
            // echo 1;
        ...
        return array('redonate' => $redonate, 'donation_link' => $donation_link, 'donation_amount' => $donation_amount);
    }
